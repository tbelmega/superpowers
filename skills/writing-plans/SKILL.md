...
- Small supporting edits outside owned files are allowed only when necessary to complete the task cleanly, consolidate evidence-supported duplication, maintain coherent contracts, or preserve the project's existing architectural boundaries and separation of concerns without implementing future-task work.
...
- The task should explicitly look for cross-task duplication, premature abstractions, unclear ownership, mixed responsibilities, misplaced code, awkward file boundaries, naming drift, and evidence-supported extractions or seams that became apparent only after the implementation landed.
...
- Encourage local engineering judgment within task scope: workers may choose better local naming, decomposition, evidence-supported abstractions, code-hygiene refactors, and small implementation improvements that serve the task's intent.
- Apply AHA and the Rule of Three when planning and reviewing abstractions: normally allow two concrete occurrences while the shared shape is uncertain; at the third, evaluate whether pattern and ownership are genuinely shared. Permit earlier abstraction only for an existing domain contract, architectural boundary, or correctness invariant, never predicted reuse. Do not force consolidation when similar-looking cases change independently.
